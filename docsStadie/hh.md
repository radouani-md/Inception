...
The `~` means Nginx uses a regular expression for matching.

The expression `\.php$` means:
...
So `wordpress` is the Docker container/service name.

And `9000` is the port where PHP-FPM is listening.
...
Here Nginx sends the request to `adminer` on `9000`.
